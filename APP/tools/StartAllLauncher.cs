...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            var runScript = Path.Combine(baseDir, "run-all.ps1");

            if (!File.Exists(runScript))
            {
                Console.Error.WriteLine("run-all.ps1 non trovato nella stessa cartella dell'exe.");
            ...
            Process.Start(psi);
            Console.WriteLine("Pipeline avviata in una nuova finestra PowerShell elevata.");
